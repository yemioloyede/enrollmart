...
var setUploader = function(path) {
	...
	$('#uploader h1').text(lg.current_folder + subDomain + displayPath(path)).attr('title', displayPath(path, false)).attr('data-path', path);
...
$(function(){
	...
	if (!($.urlParam('field_name')))
	{
	    loadCSS('/themes/default/styles/filemanager.css');
	}
	else
	{
	...
	if(!config.options.fileRoot) {
		...
		if(!config.options.serverRoot) {
			...
			fileRoot = '/' + config.options.fileRoot + '/' + subDomain + '/';
		}
	...
	if(config.upload.multiple) {
		...
		$('#upload').unbind().click(function() {
			// we create prompt
			var msg  = '<div id="dropzone-container"><h2>' + lg.current_folder + subDomain + $('#uploader h1').attr('title')  + '</h2><div id="multiple-uploads" class="dropzone"></div>';
			...
			$("div#multiple-uploads").dropzone({ 
				...
				acceptedFiles: path.indexOf("/Certificates/") >= 0 ? ".docx" : allowedFiles,
